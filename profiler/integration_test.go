...
func newClusterConfig(projectID string) clusterConfig {
	return clusterConfig{
		...
		imageName:       fmt.Sprintf("%s/profiler-test-%v", projectID, timestamp),
...
func (tr *testRunner) queryAndCheckProfile(service, startTime, endTime, profileType, projectID string) error {
	...
	const queryJsonFmt = `{"deploymentLabels": {}, "endTime": "%s", "profileLabels": {}, "profileType": "%s", 
"startTime": "%s", "target": "%s"}`
...
func (tr *testRunner) runTestOnGCE(ctx context.Context, t *testing.T, inst instanceConfig, projectID, zone string) {
	...
	if err := tr.queryAndCheckProfile(inst.service, startTime, endTime, "CPU", projectID); err != nil {
		t.Errorf("queryAndCheckProfile(%s, %s, %s, CPU) got error: %v", inst.service, startTime, endTime, err)
	}

	if err := tr.queryAndCheckProfile(inst.service, startTime, endTime, "HEAP", projectID); err != nil {
		t.Errorf("queryAndCheckProfile(%s, %s, %s, HEAP) got error: %v", inst.service, startTime, endTime, err)
	}
}

// createAndPublishDockerImage creates a docker image from source code in a GCS
// bucket and push the image to Google Container Registry.
func (tr *testRunner) createAndPublishDockerImage(ctx context.Context, projectID, sourceBucket, sourceObject, imageName string) error {
	...
	for {
		...
		case <-time.After(10 * time.Second):
			...
			if op.Done == true {
				log.Printf("Published image %v to Google Container Registry.", imageName)
				return nil
			}
		}
	}
...
func (tr *testRunner) deleteDockerImage(ctx context.Context, imageName string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to list tags for image %s: %v", imageName, err)
	}
	defer resp.Body.Close()
	...
	for _, tag := range ir.Tags {
		if err := deleteDockerImageResource(tr.client, fmt.Sprintf(deleteImageURLFmt, imageName, tag)); err != nil {
			return fmt.Errorf("failed to delete tag %s for image %s: %v", tag, imageName, err)
		}
	}
	...
	for manifest := range ir.Manifest {
		if err := deleteDockerImageResource(tr.client, fmt.Sprintf(deleteImageURLFmt, imageName, manifest)); err != nil {
			return fmt.Errorf("failed to delete manifest %s for image %s: %v", manifest, imageName, err)
		}
	}
	return nil
}
...
func (tr *testRunner) createCluster(ctx context.Context, client *http.Client, projectID, zone, clusterName string) error {
	...
	for {
		...
		case <-time.After(10 * time.Second):
			...
			if op.Status == "DONE" {
				log.Printf("Created cluster %v.", clusterName)
				return nil
			}
		}
	}
...
func (tr *testRunner) runTestOnGKE(ctx context.Context, t *testing.T, cfg clusterConfig, projectID, zone string) {
	if err := tr.uploadImageSource(ctx, *bucketName, cfg.imageSourceName, *commit, cfg.service); err != nil {
	...
	defer func() {
		if err := tr.storageClient.Bucket(*bucketName).Object(cfg.imageSourceName).Delete(ctx); err != nil {
			t.Errorf("Bucket(%s).Object(%s).Delete() got error: %v", *bucketName, cfg.imageSourceName, err)
		}
	}()

	createImageCtx, cancel := context.WithTimeout(ctx, 5*time.Minute)
	defer cancel()
	if err := tr.createAndPublishDockerImage(createImageCtx, projectID, *bucketName, cfg.imageSourceName, fmt.Sprintf("gcr.io/%s", cfg.imageName)); err != nil {
		t.Fatalf("createAndPublishDockerImage(%s) got error: %v", cfg.imageName, err)
	}
	defer func() {
		if err := tr.deleteDockerImage(ctx, cfg.imageName); err != nil {
	...
	if err := tr.queryAndCheckProfile(cfg.service, startTime, endTime, "CPU", projectID); err != nil {
		t.Errorf("queryAndCheckProfile(%s, %s, %s, CPU) got error: %v", cfg.service, startTime, endTime, err)
	}

	if err := tr.queryAndCheckProfile(cfg.service, startTime, endTime, "HEAP", projectID); err != nil {
		t.Errorf("queryAndCheckProfile(%s, %s, %s, HEAP) got error: %v", cfg.service, startTime, endTime, err)
	}

}

// uploadImageSource uploads source code for building docker image to GCS.
func (tr *testRunner) uploadImageSource(ctx context.Context, bucketName, objectName, commit, service string) error {
